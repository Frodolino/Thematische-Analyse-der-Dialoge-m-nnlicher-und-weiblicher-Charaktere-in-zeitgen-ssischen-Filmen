...
custom_stopwords <- tibble(word = c(
  ...
  "carter", "lisa", "laura", "andrew", "wendy",
  # Eigennamen Welle 5
  "bill", "eddie", "johnny", "martin", "jay", "luke", 
...
topic_assignments %>%
  ...
  summarise(mean_gamma = mean(mean_gamma), .groups = "drop") %>%
  ggplot(aes(x = gruppe, y = mean_gamma, fill = gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +
  labs(title = "Themenverteilung nach Gruppe und Gender",
       x = "Themengruppe", y = "Themenanteil",
       fill = "Gender") +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme_minimal()

df_corpus %>% count(gender)
nrow(df_joined)
